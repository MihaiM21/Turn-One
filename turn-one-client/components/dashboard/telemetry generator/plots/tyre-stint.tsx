...
export function TyreStintGraph({ data, advancedSettings }: TyreStintGraphProps) {
  ...
  return (
    <div style={{ height: `${settings.chartHeight}px`, overflowY: "auto" }}>
      <div className="min-h-full p-4 space-y-1.5">
        {/* Lap axis */}
        {settings.showGrid && (
          <div className="relative h-5 ml-[100px] mr-[44px] mb-1">
            {lapTicks.map((lap) => (
              <div
                key={lap}
            ...
            {settings.showGrid &&
              lapTicks.filter((l) => l > 0).map((lap) => (
                <div
                  key={`tick-${lap}`}
                  className="absolute top-3 bottom-0 w-px bg-zinc-800"
                  style={{ left: `${(lap / totalLaps) * 100}%` }}
                />
              ))}
          </div>
        ...
        {sortedDrivers.map(([driver, stints]) => {
          ...
          return (
            <div key={driver} className="flex items-center gap-2">
              <div className="w-[36px] text-right font-mono text-xs text-zinc-500 tabular-nums shrink-0">
                P{position}
              </div>
              <div
                className="w-[56px] shrink-0 truncate text-xs font-semibold"
                style={{ color }}
              >
                {driver}
              </div>
              <div className="relative h-6 flex-1 bg-zinc-900">
                {stints.map((s, i) => {
                  ...
                  return (
                    <div
                      key={i}
                      className="absolute inset-y-0 flex items-center justify-center text-[10px] font-bold text-black hover:opacity-80 transition-opacity"
                      style={{
                        left: `${startPct}%`,
                        width: `${widthPct}%`,
              ...
              <div className="w-[36px] text-right font-mono text-xs text-zinc-400 tabular-nums shrink-0">
                {stops}x
              </div>
            </div>
          );
        ...
        {settings.showLegend && (
          <div className="flex flex-wrap items-center gap-4 pt-4 mt-2 border-t border-zinc-800 ml-[100px]">
            ...
            <div className="flex items-center gap-1.5">
              <span className="h-3 w-0.5 inline-block bg-zinc-600" />
